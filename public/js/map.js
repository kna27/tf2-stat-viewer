{
    const mapPlayTimeStats = {};
    const vals = [];
    const names = [];
    for (const key in window.playerStats) {
        if (
            key.endsWith(".accum.iPlayTime") &&
            !window.CLASSES.some((substring) => key.includes(substring)) &&
            !key.endsWith(".mvm.accum.iPlayTime") &&
            window.playerStats[key] != 0
        ) {
            mapPlayTimeStats[key.substring(0, key.length - 16)] =
                window.playerStats[key];
        }
    }

    const items = Object.keys(mapPlayTimeStats).map(function (key) {
    ...
    const mapChartCanvas = document.getElementById("mapChart").getContext("2d");
    new Chart(mapChartCanvas, {
        type: "bar",
        data: {
            labels: names,
            datasets: [
